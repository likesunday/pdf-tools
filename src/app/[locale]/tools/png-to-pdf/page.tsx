  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">{t('tools.pngToPdf.title')}</h1>
        <p className="text-gray-600 mt-2">{t('tools.pngToPdf.description')}</p>
      </div>
      {!result ? (
        <div className="space-y-6">
          <FileUploader accept={{ 'image/png': ['.png'] }} multiple files={files} onFilesSelected={handleFiles} onRemoveFile={removeFile} />
          {files.length > 0 && (
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              ...
              <button onClick={handleConvert} disabled={processing} className="w-full bg-[#4B83FF] hover:bg-[#3A6FE0] disabled:bg-gray-300 text-white font-medium py-3 rounded-lg transition-colors mt-4">
                {processing ? t('common.processing') : t('tools.pngToPdf.title')}
              </button>
            </div>
          )}
        </div>
