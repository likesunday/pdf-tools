...
export default function DecryptPdfPage() {
  ...
  const handleDecrypt = async () => {
    ...
    try {
      ...
      setProgress(100);
      setResult(new Blob([(decrypted as any)], { type: 'application/pdf' }));
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">{t('tools.decryptPdf.title')}</h1>
        <p className="text-gray-600 mt-2">{t('tools.decryptPdf.description')}</p>
      </div>
      ...
      {!file ? (
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
          ...
          >
            {processing ? t('common.processing') : t('tools.decryptPdf.title')}
          </button>
        </div>
      ) : (
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100 text-center">
          ...
          <div className="flex gap-3 justify-center">
            <button
              onClick={() => downloadBlob(result, `decrypted_${file.name}`)}
