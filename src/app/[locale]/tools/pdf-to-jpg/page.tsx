...
export default function PdfToJpgPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setProgress(40);
      const images = await pdfToImages(buffer, { format: 'jpeg', scale: 2, quality: 0.92 });
      setProgress(100);
      setResults(images);
    } catch { /* ignore */ }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">{t('tools.pdfToJpg.title')}</h1>
        <p className="text-gray-600 mt-2">{t('tools.pdfToJpg.description')}</p>
      </div>
      {!file ? (
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
          ...
          <button onClick={handleConvert} disabled={processing} className="w-full bg-[#4B83FF] hover:bg-[#3A6FE0] disabled:bg-gray-300 text-white font-medium py-3 rounded-lg transition-colors mt-4">
            {processing ? t('common.processing') : t('tools.pdfToJpg.title')}
          </button>
        </div>
      ) : (
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100 text-center">
          <p className="text-green-600 font-medium mb-4">Converted {results.length} pages to JPG</p>
          <div className="flex gap-3 justify-center">
            <button onClick={() => results.forEach((b, i) => setTimeout(() => downloadBlob(b, `page_${i+1}.jpg`), i * 200))} className="bg-[#4B83FF] hover:bg-[#3A6FE0] text-white font-medium px-6 py-3 rounded-lg transition-colors">{t('common.downloadAll')}</button>
