  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">{t('tools.compressPdfTo2mb.title')}</h1>
        <p className="text-gray-600 mt-2">{t('tools.compressPdfTo2mb.description')}</p>
      </div>
      {!file ? (
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
          ...
          <button onClick={handleCompress} disabled={processing} className="w-full bg-[#4B83FF] hover:bg-[#3A6FE0] disabled:bg-gray-300 text-white font-medium py-3 rounded-lg transition-colors mt-4">
            {processing ? t('common.processing') : t('tools.compressPdfTo2mb.title')}
          </button>
        </div>
      ) : (
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100 text-center">
          ...
          <div className="flex gap-3 justify-center">
            <button onClick={() => downloadBlob(result.blob, `compressed_2mb_${file.name}`)} className="bg-[#4B83FF] hover:bg-[#3A6FE0] text-white font-medium px-6 py-3 rounded-lg transition-colors">{t('common.download')}</button>
